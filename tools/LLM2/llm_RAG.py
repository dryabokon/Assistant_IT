...
# ----------------------------------------------------------------------------------------------------------------------
    def make_rag_prompt(self,query, relevant_passage):
      ...
      """).format(query=query, relevant_passage=escaped)
        ...
        return prompt
# ----------------------------------------------------------------------------------------------------------------------
    def run_query(self, query:str, search_field='token',limit=4):
        ...
        texts = self.Vector_Searcher.search_vector(query, field=search_field, select=self.select, as_df=False,limit=limit)
        docs = [Document(page_content=t, metadata={}) for t in texts]
